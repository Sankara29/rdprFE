...
const Floor = ({ stepper, type, data1 }) => {
  ...
  formData.append("docrefno", "company_" + sessionStorage.getItem('companyID'));
  ...
  const onSubmit = (data) => {
    ...
    if (sessionStorage.getItem('companyExistenceCheck') !== 1 && sessionStorage.getItem('companyExistenceCheck') == null) {
      handleError("No companies are associated !!!")
    }
      ...
      let createmarketGroup = JSON.stringify({
        isBtcApproved: (data.btcApproved3 === undefined ? btcApproved[1].value : data.btcApproved3.value),
        creditLimit: data.creditLimit1,
        tenure: data.tenure1,
        attachment: data.idFile,
      });

      fetchx(API_URL + `/updateaccounts?companyid=${sessionStorage.getItem('companyID')}`, {
        method: "PUT",
