  ...
  const [rowData, setRowData] = useState();
    ...
    const updatedItem = JSON.stringify({            
      ...
      //  console.log(updatedItem)
       fetchx(API_URL+`/updateMarketGroups?id=${event.data.id}`, {
       method: 'PUT',
       body: updatedItem,
       headers: {
...
// console.log(updatedItem)
fetchx(API_URL+ `/updateMarketGroups`, {
method: 'PUT',
body: updatedItem,
headers: {
'Content-type': 'application/json'
...
setPopUp(false)
fetchx(API_URL + '/getMarketGroup?hotelID=1')
...
const [oldValue, setOldValue] = useState(null);
  ...
  const onSubmit = data => {
    ...
    if (data.marketGroup.length && data.description.length) {
      ...
      let res = fetchx(API_URL +'/addMarketGroup', {
        method: "POST",
