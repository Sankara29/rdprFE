...
const ListGroupNavigation = ({ selectedTaluk }) => {
  ...
  useEffect(() => {
      fetchx(API_URL + `/getTalukWiseData?taluk_id=${selectedTaluk.value}&data=date`)
  ...
  const [columnDefs, setColumnDefs] = useState([
      ...
      { headerName: 'Water CONSUMED(kwh) ', field: 'total_water_supply_sum', maxWidth: 120,  wrapText: true,
        headerComponentFramework: CustomHeaderComponent, cellStyle: { textAlign: 'center',fontSize: '10px', fontWeight: 'bold' },valueFormatter: params => formatValue(params.value),  filter: true, headerStyle: { fontSize: '2px', fontWeight: 'bold' }  },
  ...
  console.log(selectedTaluk.value)
  // Monthly

  const gridRef2 = useRef();
  let [rowData2, setRowData2] = useState();

  useEffect(() => {
      fetchx(API_URL + `/getTalukWiseData?taluk_id=${selectedTaluk.value}&data=month`)
  ...
  let [rowData3, setRowData3] = useState();

  useEffect(() => {
      fetchx(API_URL + `/getTalukWiseData?taluk_id=${selectedTaluk.value}&data=year`)
  ...
  const defaultColDef3 = useMemo(() => ({
    ...
    return (
        <Card style={{ display: 'flex', flexDirection: 'row', alignItems: 'center',width:'360px', marginLeft:'-70px' }}>
            <CardBody>
