...
const OverView = ({ node_id, date, rr_no, GP }) => {
    ...
    const columnDefsWater = useMemo(() => [
        {
            ...
            maxWidth: 180, valueFormatter: (params) =>
                params.data.datetime
                    ? dayjs(params.data.datetime).format('MMM-DD-YYYY HH:MM')  // Format date and time
                    : 'N/A'  // Fallback in case of missing value
        ...
        },
        {
            ...
            valueFormatter: (params) =>
                params.data.billingdate
                    ? dayjs(params.data.billingdate).format('MMM-DD-YYYY')  // Format date and time
                    : 'N/A'  // Fallback in case of missing value
        },
    ...
    const handleStart = async () => {
        try {
            const response = await fetch(`https://api.ms-tech.in/v14/setcommandtogateway?gwid=${node_id}&command_info=N|${node_id}|0|1`, {
                method: 'GET',
                headers: {
    ...
    const handleStop = async () => {
        try {
            const response = await fetch(`https://api.ms-tech.in/v14/setcommandtogateway?gwid=${node_id}&command_info=N|${node_id}|0|0`, {
                method: 'GET', // Change to correct method for stop
                headers: {
                    'Content-Type': 'application/json'
                },
            });
    ...
    return (
        <div style={{ width: '100%', height: '100%' }}>
            ...
            <div className="nodeDetails">
                ...
                {!loading && !error && filteredData.length === 0 && <div className="ag-theme-alpine" style={{ height: '14px', width: '100%' }}>
                    <Loader />
                </div>}
                {waterData.length > 0 && (
                    <>
